...
router.get('/stats', authenticateToken, async (req, res, next) => {
  try {
    ...
    const recentInterviews = await Interview.find({ user: userId, status: 'completed' })
      .select('topic overallScore completedAt type difficulty').sort({ completedAt: -1 }).limit(5);

    const bestInterview = await Interview.findOne({ user: userId, status: 'completed' })
      .sort({ overallScore: -1 }).select('topic overallScore completedAt');

    const worstInterview = await Interview.findOne({ user: userId, status: 'completed' })
      .sort({ overallScore: 1 }).select('topic overallScore completedAt');

    res.json({ success: true, data: { profile: user, recentInterviews, bestInterview, worstInterview } });
  } catch (err) { next(err); }
});
